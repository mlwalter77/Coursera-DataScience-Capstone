...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        successCounts = filtered_df.value_counts()
        fig = px.pie(filtered_df, names= 'class', values= successCounts, 
...
def get_scatter_chart(entered_site, slider_range):
    ...
    if entered_site == 'ALL':
        scatter_df = spacex_df[(spacex_df['Payload Mass (kg)'] > low) & (spacex_df['Payload Mass (kg)'] < high)] 
        fig = px.scatter(scatter_df, x='Payload Mass (kg)', y = 'class', color = 'Booster Version Category')
        return fig
    else:
        # return the outcomes piechart for a selected site
        filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site]
        scatter_df = filtered_df[(filtered_df['Payload Mass (kg)'] > low) & (filtered_df['Payload Mass (kg)'] < high)]
        fig = px.scatter(scatter_df, x='Payload Mass (kg)', y = 'class', color = 'Booster Version Category')
        return fig
